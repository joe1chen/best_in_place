...
BestInPlaceEditor.prototype = {
  ...
  initOptions : function() {
    ...
    self.element.parents().each(function(){
      ...
      self.cleditor_doctype    = self.cleditor_doctype    || jQuery(this).attr("data-cleditor-doctype");
      self.cleditor_doccssfile = self.cleditor_doccssfile || jQuery(this).attr("data-cleditor-doccssfile");
      self.cleditor_bodystyle  = self.cleditor_bodystyle  || jQuery(this).attr("data-cleditor-bodystyle");
      self.cleditor_imagespath  = self.cleditor_imagespath  || jQuery(this).attr("data-cleditor-imagespath");
    });
    ...
    self.cleditor_doctype    = self.element.attr("data-cleditor-doctype") || self.cleditor_doctype;
    self.cleditor_doccssfile = self.element.attr("data-cleditor-doccssfile") || self.cleditor_doccssfile;
    self.cleditor_bodystyle  = self.element.attr("data-cleditor-bodystyle") || self.cleditor_bodystyle;
    self.cleditor_imagespath  = self.element.attr("data-cleditor-imagespath") || self.cleditor_imagespath;
...
BestInPlaceEditor.forms = {
  ...
  "cleditor" : {
    activateForm : function() {
      var bipSpan = this.element;
      bipSpan.css({'display':'inline','overflow':'visible'});

      // construct the form
      var output = '<form action="javascript:void(0)" style="display:inline;"><textarea>';
      if (this.display_value != this.nil) {
        output += this.display_value;
      }
      output += '</textarea><a class="finish-editing" href="javascript:void(0);">Finish Editing</a></form>';
      bipSpan.html(output);
      this.setHtmlAttributes();

      // Bind the finish link
      bipSpan.find("a.finish-editing").bind('click', {editor: this}, BestInPlaceEditor.forms.cleditor.blurHandler);

      // Set min-height of text area.
      var ta = bipSpan.find("textarea");
      ta.css({ 'min-height': (bipSpan.height() > 200 ? bipSpan.height() : 200) });
      ta.elastic();

      //this.element.find("textarea").bind('blur', {editor: this}, BestInPlaceEditor.forms.cleditor.blurHandler);
      //this.element.find("textarea").bind('keyup', {editor: this}, BestInPlaceEditor.forms.cleditor.keyupHandler);
      ...
      if (this.cleditor_doctype) {
        cleditorOptions.docType = this.cleditor_doctype;
      }
      if (this.cleditor_doccssfile) {
        cleditorOptions.docCSSFile = this.cleditor_doccssfile;
      }
      if (this.cleditor_bodystyle) {
        cleditorOptions.bodyStyle = this.cleditor_bodystyle;
      }
      if (this.cleditor_imagespath) {
        cleditorOptions.imagesPath = this.cleditor_imagespath;
      }

      var theEditor = ta.cleditor(cleditorOptions)[0].focus();

      // Set iframe height
      setTimeout(function() {
        var body = bipSpan.find("iframe").contents().find("body");
        if (body) {
          bipSpan.find("iframe").css({'min-height':ta.height(), 'height':body.height()+20, 'width':ta.width()});

          // Add resize event so when iframe body changes, then the iframe size changes.
          if (body && typeof body.resize == 'function') {
            body.resize(function() {
              bipSpan.find("iframe").css({'height':$(this).height()+20});
            });
          }
        }
      }, 100);
    ...
    getValue :  function() {
      if (this.element.find("textarea").val() == "<br/>") {
        return "";
      }
    ...
    blurHandler : function(event) {
      ...
      event.data.editor.element.css({'display':'block','overflow':'auto'});
    },
