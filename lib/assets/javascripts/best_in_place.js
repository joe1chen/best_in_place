...
BestInPlaceEditor.forms = {
    ...
    "cleditor" : {
        activateForm : function() {
            ...
            this.element.find("textarea").cleditor({width:"100%", height:"100%"});
            this.element.find("a.finish-editing").bind('click', {editor: this}, BestInPlaceEditor.forms.cleditor.blurHandler);
            this.element.find("iframe").css({'min-width':width, 'min-height':height});
            if (typeof this.element.find("iframe").iframeAutoHeight == 'function') {
                this.element.find("iframe").iframeAutoHeight({heightOffset: 10});
            }
        },
